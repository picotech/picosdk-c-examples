...
void setTimebase(GENERICUNIT* unit)
{
	...
	unit->timeInterval = timeInterval * 1e-9;
}
...
void displaySettings(GENERICUNIT* unit)
{
	...
	printf("\nReadings will be scaled in %s\n", (scaleVoltages) ? ("Volts") : ("ADC counts"));
	printf("\n");
...
PICO_STATUS handleDevice(GENERICUNIT* unit)
{
	...
	do
	{
		status = ps6000aGetTimebase(unit->handle, timebase, BUFFER_SIZE, &temp_timeIntervalns, NULL, 0);
	...
	for (i = 0; i < unit->channelCount; i++)
	{
		...
		unit->channelSettings[i].bandwithLimit = PICO_BW_FULL;
	}
