		...
		if (unit->channelSettings[channel].enabled)
		{
			for (capture = 0; capture < nCaptures; capture++)
			{
				status = ps6000aSetDataBuffers(unit->handle,
					...
					maxBuffers[channel][capture],
					minBuffers[channel][capture],
					(int32_t)nSamples,
