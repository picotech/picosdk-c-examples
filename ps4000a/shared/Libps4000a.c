 ...
 *
 * Copyright (C) 2023-2026 Pico Technology Ltd. See LICENSE file for terms.
 *
 ******************************************************************************/
...
/****************************************************************************
 * ProbestoSettings -  Copy probe settings to unit struct for use in setting channels to most recent
 ****************************************************************************/
void ProbestoSettings(GENERICUNIT* unit) {
    int16_t i;

    /*******************************   DEBUG   ************************************/
    printf("DEBUG Called - ProbestoSettings() - copied probe info to settings\n");

    for (i = 0; i < unit->channelCount; i++)
    {
        if (unit->channelSettings[PS4000A_CHANNEL_A + i].enabled == TRUE)
        {
            if (userProbeInfo.userProbeInteractions[i].connected_ == TRUE)
            {
                unit->channelSettings[PS4000A_CHANNEL_A + i].range = userProbeInfo.userProbeInteractions[i].rangeCurrent_;
                unit->channelSettings[PS4000A_CHANNEL_A + i].DCcoupled = userProbeInfo.userProbeInteractions[i].couplingCurrent_;
            }
        }
        else // channel disabled (only change channel range if a probe is connected)
        {
            if (userProbeInfo.userProbeInteractions[i].connected_ == TRUE)
            {
...
PICO_STATUS SetTrigger(GENERICUNIT *unit,
  ...
  if ((status = ps4000aSetTriggerChannelProperties(
           ...
           auxOutputMode, // Correctly pass auxOutputMode here if needed, or 0.
                          // ps6000a passed 0 and called SetAuxIoMode. ps4000a
                          // has auxOutputEnable here.
...
void set_info(GENERICUNIT *unit) {
  ...
  if (unit->handle) {
    ...
    case MODEL_4444: /* ... */
        ...
        break;
    case MODEL_4225A: /* ..Automotive Scope.. */
        ...
        //unit->sigGen	= SIGGEN_NONE;
        break;
    case MODEL_4425A: /* ..Automotive Scope.. */
        printf("Model is 4225A\n");
      ...
      for (ch = 0; ch < unit->channelCount; ch++)
      {
          ...
          do
          {
                  ...
                  {   //Print Probe name and its current range
                      ...
                      //printf("\t\tProbe Range : %d\n", userProbeInfo.userProbeInteractions[ch].rangeCurrent_);//ONLY UPDATES WITH CALLBACKS
                      printf("\t\tProbe Range : %d\n", unit->channelSettings[ch].range); // USE range from settings applied
                  }
                  else //Or standard voltage range
                      printf("Current voltage range %d\n", unit->channelSettings[ch].range);

                  _firstRange = unit->firstRange;
                  _lastRange = unit->lastRange;

                  //Print available, standard voltage ranges OR PicoConnectProbes ranges
                  if ((userProbeInfo.userProbeInteractions[ch].probeName_ == PICO_CONNECT_PROBE_NONE) )
                  {
                      //muiltple enums for the same value, just cast to standard voltage ranges-
					  printf("Specify voltage range (%d..%d)", (int)unit->firstRange, (int)unit->lastRange);// print ranges for user input
                      printf("                Enter (%d..%d)", (int)unit->firstRange, (int)unit->lastRange);
                  }
                  else
                  {
                      ...
                      if (userProbeInfo.userProbeInteractions[ch].rangeLast_ <= PICO_X1_PROBE_50V)
                          ...
                          printf("Specify range (%d..%d))", userProbeInfo.userProbeInteractions[ch].rangeFirst_, userProbeInfo.userProbeInteractions[ch].rangeLast_);
                      printf("        Enter (%d..%d))", (int)userProbeInfo.userProbeInteractions[ch].rangeFirst_, (int)userProbeInfo.userProbeInteractions[ch].rangeLast_);
                  }
...
void setResolution(GENERICUNIT *unit) {
  ...
  do {
    ...
    if (newResolution == PICO_DR_12BIT && numEnabledChannels > 2) {
      printf("setResolution: 12 bit resolution can only be selected with 2 "
             "channel enabled.\n");
    } else if (newResolution == PICO_DR_10BIT && numEnabledChannels > 4) {
      printf("setResolution: 10 bit resolution can only be selected with a "
             "maximum of 4 channels enabled.\n");
  ...
  status = ps4000aSetDeviceResolution(unit->handle,
                                      (PICO_DEVICE_RESOLUTION)newResolution);
  ...
  if (status == PICO_OK) {
    ...
    // The maximum ADC value will change if transitioning from 8 bit to >= 12
    // bit or vice-versa
  ...
  for (ch = 0; ch < unit->channelCount; ch++) {
    if (!(unit->channelSettings[ch].enabled)) {
      printf("Channel %c Range: Off\n", 'A' + ch);
        ...
        {
            //printf("ProbeRangeEnum: %d Probe Units?, ", unit->channelSettings[ch].range);
            getRangeScaling(unit->channelSettings[PS4000A_CHANNEL_A + 0].range, &chRangeInfoTemp);
            printf("ProbeRange: %d Probe Units: %s, ", unit->channelSettings[ch].range, chRangeInfoTemp.Unit_text);
        }
        else
        {
...
void SetupTrigger(GENERICUNIT *unit) {
  ...
  struct tPS4000ATriggerChannelProperties sourceDetails = {
      ...
      PS4000A_CHANNEL_A,  // channel - PICO_CHANNEL
      PS4000A_LEVEL
  };

  struct tPS4000ACondition conditions = {
      sourceDetails.channel, // PICO_CHANNEL
  ...
  //directions.thresholdMode = PS4000A_LEVEL

  // Create Pulse Width Qualifier structure with settings
  struct tps4000aPwq pulseWidth;
  memset(&pulseWidth, 0, sizeof(struct tps4000aPwq)); // zero out pulseWidth
      ...
      for (capture; capture < nCaptures; capture++) {
        ...
        if (status != PICO_OK) {
          printf("SetAllDataBuffers:ps4000aSetDataBuffers ------ 0x%08x, for "
                 "channel %c \n",
                 status, PS4000A_CHANNEL_A + channel);
          return;
        }
        // if (capture == 0)
        //	printf("%c,", 'A' + channel);
